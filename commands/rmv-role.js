...
module.exports.run = (message, args, client, time) => {
    ...
    if (member.roles.cache.some(role => role.id === NewRole.id)) {
        ...
        message.channel.send(new Discord.MessageEmbed()
            .setColor('#664DFF')
            .setDescription(`Le role ${NewRole} a été retiré de ${member}`))
        console.log(`${time} La commande "${config.prefix}rmv-role" a été utilisée par ${message.author.username} (id:${message.author.id}) dans le salon #${message.channel.name}, le role "${NewRole.name}" a été retiré de ${member.displayName} (id:${member.id})`);
        client.channels.cache.get(config.channelLogs).send(new Discord.MessageEmbed()
            .setColor('#664DFF')
            .setDescription(`${time} La commande "${config.prefix}rmv-role" a été utilisée par ${message.author} (id:${message.author.id}) dans le salon ${message.channel}, le role ${NewRole} a été retiré de ${member} (id:${member.id})`))
    } else {
        message.channel.send(new Discord.MessageEmbed()
            .setColor('#664DFF')
            .setDescription(`${member} ne possede pas le role ${NewRole} !`))
    }
}
